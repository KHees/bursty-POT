...
abstract: |
  ...
  above high thresholds is proposed; the threshold exceedances itself are modeled via the standard 
...
$$
\frac{f(\lambda x)}{f(x)} = \lambda^\rho \quad \text{ for all } \lambda > 0.
$$
...
p1<-hill_plot_values %>% ggplot() + geom_line(aes(x=x,y=y,group=rep),colour="lightgrey") +
...
p2<-hill_plot_values %>% ggplot() + geom_line(aes(x=x,y=y,group=rep),colour="lightgrey") +
  coord_cartesian(ylim = c(0.5, 1.5)) +
...
```{r LRT_power, eval=FALSE, echo=FALSE,warning=FALSE, fig.align="center", fig.cap="\\label{Fig:LRT} Power for bootstrapped LRT for different sample sizes, varying tails and scale parameter equal to 1.", out.width='90%'}
...
Hence CTRE and Cluster Poisson Process shouldn't be viewed as competing methods, as the underlying data generating processes are quite different.
In weighing the evidence for either of the two data generating processes, criteria could be developed, based e.g. on measures of surprise [@Lee15], which may prove to be valuable for future applied statisticians.
